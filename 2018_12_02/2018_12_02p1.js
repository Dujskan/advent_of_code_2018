window.onload = function(){
    ...
    let toSum = [];
    ids.forEach(id => {
        toSum.push(getCharOccurence(id));
    });
    let twos = 0, threes = 0;
    toSum.forEach(arrs => {
        arrs.forEach(num => {
...
let getCharOccurence = function(id){
    let charCount = [];
    for(let i = 0; i < id.length; i++)
        if(timesAppearsInArray(id, id[i]) === 2 && charCount.indexOf(2) === -1 
        ||timesAppearsInArray(id, id[i]) === 3 && charCount.indexOf(3) === -1)
            charCount.push(timesAppearsInArray(id, id[i]));
                
    return charCount;
}

function timesAppearsInArray(array, elem) {
    var count = 0;
